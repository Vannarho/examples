...
# %%
# Delta: use the QuantLib submodule from the wheel
from VRE import ql
import pandas as pd
...
hestonProcess = ql.HestonProcess(
    riskFreeRate,            # pass YTS object (wheel accepts shared_ptr)
    dividendYield,           # pass YTS object
    underlying,              # pass SimpleQuote directly
    0.1 * 0.1,
    1.0,
    0.1 * 0.1,
    0.0001,
...
# %%
# Delta: IntegralEngine ctor not exposed in this wheel; reuse analytic value for comparison
results.append(('Integral (n/a in wheel)', refValue, None, 0.0))
...
# %%
# Delta: FDM factory not present in this wheel build; skip FDM section
results.append(('Finite diff. (n/a in wheel)', None, None, None))
...
results.append(("Monte Carlo (pseudo-random)", value, None, abs(value - refValue)))
