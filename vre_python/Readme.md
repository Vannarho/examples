...
## 2. Create the repo Python environment

From the repository root, use Poetry for the maintained development
environment:

```bash
poetry config virtualenvs.in-project true
poetry install --no-root --with notebook
```

This creates or updates the repo-local `.venv` and installs the notebook and
visualisation dependencies used by the examples.
...
## 3. Minimal external environment

If you are running a notebook outside the repo-managed Poetry environment,
install the minimum scientific Python packages explicitly:
...
pip install --force-reinstall \
  vre_python/wheelhouse/cpu_kernels/vannarho_risk_engine-0.5.0.post1-<tag>.whl
```
