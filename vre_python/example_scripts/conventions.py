...
# Delta (pybind examples policy): Make data path robust regardless of CWD.
# Wheel is source of truth; adjust Python only.
...
# A - Populate the vre::data::Conventions container
...
_examples_root = os.path.abspath(os.path.join(_here, os.pardir))
_candidate_conventions = [
    os.path.join(_here, "input", "conventions.xml"),
    os.path.join(
        _examples_root,
        "notebooks",
        "example_8",
...
convFile = next((path for path in _candidate_conventions if os.path.exists(path)), None)
if convFile is None:
    searched = "\n  ".join(_candidate_conventions)
    raise FileNotFoundError(f"Could not find bundled conventions.xml. Searched:\n  {searched}")
print("Loading conventions from:", os.path.relpath(convFile, _examples_root))
...
# Delta: fixedDayCounter() returns a string in this wheel and the getter signature is inconsistent.
# For USD FedFunds examples use Actual/360.
_dc = _ql.Actual360()

flatForwardUSD = FlatForward(Date(11, 9, 2018), 0.005, _dc)
termStructureUSD = RelinkableYieldTermStructureHandle()
termStructureUSD.linkTo(flatForwardUSD)
# Delta: parser expects a different YTS proxy type; use direct index constructor with our YTS handle
parsed_idx = _ql.FedFunds(termStructureUSD)
