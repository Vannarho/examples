...
depoFuturesSwapCurve = None
depoFuturesSwapCurveError = None
try:
    depoFuturesSwapCurve = ql.PiecewiseFlatForward(settlementDays, calendar, helpers, ql.Actual360())
except Exception as exc:
    depoFuturesSwapCurveError = exc
...
depoFraSwapCurve = ql.PiecewiseFlatForward(settlementDays, calendar, helpers, ql.Actual360())
if ARGS.debug_pillars:
    try:
        dfp = _pillars_df(helpers)
...
if ARGS.curve in ('futures','both'):
    try:
        if depoFuturesSwapCurve is None:
            raise RuntimeError(f"futures curve unavailable in this wheel: {depoFuturesSwapCurveError}")
        if ARGS.debug_pillars:
            dfp = _pillars_df(futuresShortEndHelpers + swapHelpers[1:])
            if not dfp.empty:
